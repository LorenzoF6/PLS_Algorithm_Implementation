...
% Validation
runs = 100; % number of run to validate the model
num_class = 3; % error class for this dataset
flag = false; % flag = true -> run PLS2 / flag = false -> run PLS1    
[mce_cross_matrix,order_index,order_table] = PLS.orderAnalysis(X,Y,runs,flag,num_class);
[mce_ord,idOrd] = max(order_index(:,2));

% Prediction and Y_hat transformation to binary class output
[B2,~] = PLS.pls(X, Y, order_index(idOrd,1), flag);
Y_hat = normalize(X)*B2;
mce = PLS.performMCE(PLS.performClassification(Y_hat,num_class),Y); % estimate MCE

% Plotting Area
PLS.plotResult(X,Y_hat,order_index,mce_cross_matrix)
